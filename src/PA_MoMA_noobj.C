...
int main(int argc, char *argv[])
{
	...
	char wildFluxFileName[maxFilenameLength];
	sprintf(wildFluxFileName,"%s.wild",modelName);
	ifstream wildFluxFile(wildFluxFileName,ios::in);
	...
	if (!wildFluxFile)
	{
		fprintf(stderr, "Could not open wildtype flux distribution file %s!!!\n",wildFluxFileName);
		return 1;
	}
	if (!lbFile)
	{
	...
	PA_doc=readSBML(modelFileName);
  	int errors = PA_doc->getNumWarnings() + PA_doc->getNumErrors() + PA_doc->getNumFatals();

  	if (errors > 0)
  	{
		cerr << "Error(s): " << errors       << endl;
		cerr << endl;
    		PA_doc->printWarnings(cerr);
    		PA_doc->printErrors(cerr);
    		PA_doc->printFatals(cerr);
		return 1;
  	}
	else
	...
	/* Don't do an FBA, instead just get the wild-type distrib */
	...
	//Wild Flux file does exist
	double coeff;
	for (int k=1; k<=nReactions; k++)
	{
		if(wildFluxFile.eof())
		{
			cerr << "Error in Wild type flux file!" << endl;
			exit(1);
		}
		wildFluxFile >> x[k];
		k++;
...
/*
Oh.. without an obj fn.. we have viability computation problems :(
*/
		for (int i=0; i<n; i++)
		{
			fval+=f[i+1]*q[i];
